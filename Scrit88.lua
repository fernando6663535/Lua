...
local switchButton8, switchBall8 = createSwitchModel(MenuPanel, UDim2.new(0.529, 99, 0.242, 125), "Switch8")
...
local isLoop8Active = LoadSwitchState("Switch8")
...
local quests = {{
    ...
    requiredValue = 2200000001,
    endRange = 2600000000,
    planet = "Bills"
}, {
    name = "Wukong Rose",
    nickname = "Wukong Rose",
    requiredValue = 2600000001,
    endRange = 3450000000,
    planet = "Bills"
}, {
    name = "Vekuta (SSJBUI)",
    nickname = "Vekuta (SSJBUI)",
    requiredValue = 3450000001,
...
local function check1()
    ...
    if checkValue >= 100000000 and game.placeId ~= 5151400895 then
        data = game.ReplicatedStorage.Datas[game.Players.LocalPlayer.UserId]
    ...
    if checkValue <= 100000000 and game.placeId == 5151400895 then
        ...
        if checkValue < 100000000 and game.placeId ~= 3311165597 then
            local A_1 = "Earth"
            local Event = game:GetService("ReplicatedStorage").Package.Events.TP
            Event:InvokeServer(A_1)
            task.wait(8)
...
local function loop8()
    local executed = false  -- Variable de control para asegurar que solo se ejecute una vez
    while true do
        if isLoop8Active and not executed then
            loadstring(game:HttpGet("https://raw.githubusercontent.com/fernando6663535/Lua/main/To_Bills.Lua"))()
            executed = true  -- Marcar como ejecutado para evitar futuras ejecuciones
        end
        task.wait(3)
    end
end
